...
timeDependentB::timeDependentB(const char *name, const char *title, RooAbsReal &costhetal, RooAbsReal &costhetak,
                               ...
                               RooAbsReal &J6s, RooAbsReal &J7, RooAbsReal &J8, RooAbsReal &J9, RooAbsReal &h1s,
...
timeDependentB::timeDependentB(timeDependentB const &other, const char *name):
        ...
        x_("x", this, other.x_),
        y_("y", this, other.y_),
        J1s_("SJ1s", this, other.J1s_),
        J1c_("SJ1c", this, other.J1c_),
        J2s_("SJ2s", this, other.J2s_),
        J2c_("SJ2c", this, other.J2c_),
        J3_("SJ3", this, other.J3_),
        J4_("SJ4", this, other.J4_),
        J5_("DJ5", this, other.J5_),
        J6s_("DJ6s", this, other.J6s_),
        J7_("SJ7", this, other.J7_),
        J8_("DJ8", this, other.J8_),
        J9_("DJ9", this, other.J9_),
...
inline double timeDependentB::evaluate_prob(double cosThetaL, double cosThetaK, double phi, double t, double x, double y,
    ...
    auto helper = [=](double Ji, double si, double hi) {
        return (Ji * (cosh_yt + cos_xt) - 0.5 * (hi * sinh_yt + si * sin_xt));
    };

    return c * decay * (
            helper(J1s, s1s, h1s) * sinThetaK2 +
            helper(J1c, s1c, h1c) * cosThetaK2 +
            helper(J2s, s2s, h2s) * sinThetaK2 * cos2ThetaL +
            helper(J2c, s2c, h2c) * cosThetaK2 * cos2ThetaL +
            helper(J3, s3, h3) * sinThetaK2 * sinThetaL2 * cos(2 * phi) +
            helper(J4, s4, h4) * sin2ThetaK * sin2ThetaL * cos(phi) +
            helper(J5, s5, h5) * sin2ThetaK * sinThetaL * cos(phi) +
            helper(J6s, s6, h6) * sinThetaK2 * cosThetaL +
            helper(J7, s7, h7) * sin2ThetaK * sinThetaL * sin(phi) +
            helper(J8, s8, h8) * sin2ThetaK * sin2ThetaL * sin(phi) +
            helper(J9, s9, h9) * sinThetaK2 * sinThetaL2 * sin(2 * phi)
    );
}
...
timeDependentBBar::timeDependentBBar(const char *name, const char *title, RooAbsReal &costhetal, RooAbsReal &costhetak,
                               RooAbsReal& phi, RooAbsReal& t, RooAbsReal &x, RooAbsReal &y, RooAbsReal &J1sBar, RooAbsReal &J1cBar,
...
timeDependentBBar::timeDependentBBar(timeDependentBBar const &other, const char *name):
        ...
        x_("x", this, other.x_),
        y_("y", this, other.y_),
        J1sBar_("SJ1s", this, other.J1sBar_),
        J1cBar_("SJ1c", this, other.J1cBar_),
        J2sBar_("SJ2s", this, other.J2sBar_),
        J2cBar_("SJ2c", this, other.J2cBar_),
        J3Bar_("SJ3", this, other.J3Bar_),
        J4Bar_("SJ4", this, other.J4Bar_),
        J5Bar_("DJ5", this, other.J5Bar_),
        J6sBar_("DJ6s", this, other.J6sBar_),
        J7Bar_("SJ7", this, other.J7Bar_),
        J8Bar_("DJ8", this, other.J8Bar_),
        J9Bar_("DJ9", this, other.J9Bar_),
        h1s_("h1s", this, other.h1s_),
        h1c_("h1c", this, other.h1c_),
        h2s_("h2s", this, other.h2s_),
        h2c_("h2c", this, other.h2c_),
...
inline double timeDependentBBar::evaluate_prob(double cosThetaL, double cosThetaK, double phi, double t, double x, double y,
    ...
    auto helper = [=](double JiBar, double si, double hi) {
        return (JiBar * (cosh_yt + cos_xt) - 0.5 * (hi * sinh_yt - si * sin_xt));
    };

    return c * decay * (
            helper(J1sBar, s1s, h1s) * sinThetaK2 +
            helper(J1cBar, s1c, h1c) * cosThetaK2 +
            helper(J2sBar, s2s, h2s) * sinThetaK2 * cos2ThetaL +
            helper(J2cBar, s2c, h2c) * cosThetaK2 * cos2ThetaL +
            helper(J3Bar, s3, h3) * sinThetaK2 * sinThetaL2 * cos(2 * phi) +
            helper(J4Bar, s4, h4) * sin2ThetaK * sin2ThetaL * cos(phi) +
            helper(-J5Bar, s5, h5) * sin2ThetaK * sinThetaL * cos(phi) +
            helper(-J6sBar, s6, h6) * sinThetaK2 * cosThetaL +
            helper(J7Bar, s7, h7) * sin2ThetaK * sinThetaL * sin(phi) +
            helper(-J8Bar, s8, h8) * sin2ThetaK * sin2ThetaL * sin(phi) +
            helper(-J9Bar, s9, h9) * sinThetaK2 * sinThetaL2 * sin(2 * phi)
    );
}
...
//    RooRealVar J1s("J1s", "J1s", 0.437, 0, 10);
    RooRealVar J1c("J1c", "J1c", 0.437, -10, 10);
...
//    RooRealVar J1sBar("J1sBar", "J1sBar", 0.437, 0, 10);
    RooRealVar J1cBar("J1cBar", "J1cBar", 0.437, -10, 10);
    ...
    RooFormulaVar J1s("J1s", "("
                             ...
                             ") / (24*(1-y**2)*(2+x**2))",
                             RooArgList(J1c, J2s, J2c, h1s, h1c, h2s, h2c, s1s, s1c, s2s, s2c));
    ...
    RooFormulaVar J1sBar("J1sBar", "("
                             ...
                             "y*(2*h2c+4*h2s-6*h1c-12*h1s)*(1+x**2)*(1-y**2)"
                             ") / (24*(1-y**2)*(2+x**2))",
                      RooArgList(J1cBar, J2sBar, J2cBar, h1s, h1c, h2s, h2c, s1s, s1c, s2s, s2c));
    ...
    timeDependentB b("b", "b", cosThetaL, cosThetaK, phi, t, x, y, J1s, J1c, J2s,J2c,J3, J4,
                     J5, J6s, J7, J8,J9, h1s, h1c, h2s, h2c, h3,h4, h5, h6, h7, h8, h9,
                     s1s, s1c, s2s, s2c, s3,s4, s5, s6, s7, s8, s9);

    timeDependentBBar bBar("bBar", "bBar", cosThetaL, cosThetaK, phi,t, x, y, J1sBar, J1cBar, J2sBar,J2cBar,J3Bar, J4Bar,
                     J5Bar, J6sBar, J7Bar, J8Bar,J9Bar, h1s, h1c, h2s, h2c, h3,h4, h5, h6, h7, h8, h9,
                     s1s, s1c, s2s, s2c, s3,s4, s5, s6, s7, s8, s9);

    RooSimultaneous simPdf("simPdf", "Simultaneous PDF", type);
    simPdf.addPdf(b, "B0");
    simPdf.addPdf(bBar, "B0Bar");
...
//                  RooFit::Verbose(false),
                  ...
                  FitOptions(Strategy(2), Hesse(true), Save(true), PrintEvalErrors(0), EvalBackend("legacy"), NumCPU(10))
    );
...
timeDependent::timeDependent(const char *name, const char *title, RooAbsReal &costhetal, RooAbsReal &costhetak,
                                     RooAbsReal& phi, RooAbsReal& t, RooAbsReal &x, RooAbsReal &y, RooAbsReal &J1s, RooAbsReal &J1c,
                                     RooAbsReal &J2s, RooAbsReal &J2c, RooAbsReal &J3, RooAbsReal &J4, RooAbsReal &J5,
...
inline double timeDependent::evaluate_prob(double cosThetaL, double cosThetaK, double phi, double t, double x, double y,
                                           double J1s, double J1c, double J2s, double J2c, double J3, double J4, double J5, double J6s, double J7, double J8, double J9,
                                           double J1sBar, double J1cBar, double J2sBar, double J2cBar, double J3Bar, double J4Bar, double J5Bar, double J6sBar, double J7Bar, double J8Bar, double J9Bar,
                                               double h1s, double h1c, double h2s, double h2c, double h3, double h4, double h5, double h6, double h7, double h8, double h9,
                                               double s1s, double s1c, double s2s, double s2c, double s3, double s4, double s5, double s6, double s7, double s8, double s9) const {
...
double timeDependent::evaluate() const {
    return evaluate_prob( cosThetaL_,  cosThetaK_,  phi_,  t_,  x_,  y_,
                          J1s_,  J1c_,  J2s_,  J2c_,  J3_,  J4_,  J5_,  J6s_,  J7_,  J8_,  J9_,
                          J1sBar_,  J1cBar_,  J2sBar_,  J2cBar_,  J3Bar_,  J4Bar_,  J5Bar_,  J6sBar_,  J7Bar_,  J8Bar_,  J9Bar_,
                          h1s_,  h1c_,  h2s_,  h2c_,  h3_,  h4_,  h5_,  h6_,  h7_,  h8_,  h9_,
...
void timeDependent::doEval(RooFit::EvalContext &ctx) const {
    std::span<const double> cosThetaLSpan = ctx.at(cosThetaL_);
    std::span<const double> cosThetaKSpan = ctx.at(cosThetaK_);
    std::span<const double> tSpan = ctx.at(t_);
    std::span<const double> phiSpan = ctx.at(phi_);
